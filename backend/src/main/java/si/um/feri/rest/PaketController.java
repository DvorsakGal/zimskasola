...
public class PaketController {
    ...
    public Response getPaketById(@PathParam("id") int id) {
        ...
        if (paket == null) {
            log.info(() -> "/paketi/" + id + " ; Nega paketa mutec");
            return Response.status(Response.Status.NOT_FOUND).entity("Nega paketa mutec").build();
        }
    ...
    @Path("/{id}")
    @Transactional
    public Response putPaket(@PathParam("id") long id, PaketDto dto) {
        Paket paket = paketRepository.findById(id);
        if (paket == null) {
            log.info(() -> "/paketi/" + id + " ; Paket ni najden");
            return Response.status(Response.Status.NOT_FOUND).entity("Paket ni najden").build();
        }
    ...
    @Path("/{id}")
    @Transactional
    public Response deletePaket(@PathParam("id") long id) {
        Paket paket = paketRepository.findById(id);
        if (paket == null) {
            log.info(() -> "/paketi/" + id + " ; Paket ni najden");
            return Response.status(Response.Status.NOT_FOUND).entity("Paket ni najden").build();
        ...
        return Response.ok("Paket izbrisan").build();
    }
